...
export class IntensifyController {
    public async intensify(req: Request, res: Response) {
        if (!req.files || Object.keys(req.files).length === 0) {
            return res.status(400).send("No files were uploaded.");
        }

        const uploadedFile = req.files.emoji;
        //const animatedImage = ImageTransformer.intensifyImage(uploadedFile);
        console.log(uploadedFile);
        console.log(uploadedFile.data);
        console.log(uploadedFile.data.length);

        // const decodedBuffer = new Buffer(uploadedFile.data, "base64");
        ...
        // uploadedFile.mv("emoji.png", function (err) {
